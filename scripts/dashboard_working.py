...
st.markdown('<style>.header{background:white;padding:20px;border-radius:10px;margin-bottom:20px;border:1px solid #ddd;}.header h1{margin:0;color:#1a1a2e;}.header h1 span{color:#4a6cf7;}.card{background:white;padding:20px;border-radius:10px;margin:10px 0;border:1px solid #ddd;}.metric{background:white;padding:15px;border-radius:10px;text-align:center;border:1px solid #ddd;}.stButton>button{width:100% !important;border-radius:8px !important;font-weight:500 !important;}.stButton>button[kind="primary"]{background:#4a6cf7 !important;color:white !important;}.stButton>button[kind="secondary"]{background:#e8e8e8 !important;color:#1a1a2e !important;}.notification{padding:10px 15px;border-radius:8px;margin:10px 0;background:#e8f5e9;border-left:4px solid #34c759;}.notification.error{background:#fbe9e7;border-left-color:#ff3b30;}.webcam-container{background:#1a1a2e;border-radius:10px;padding:10px;min-height:300px;display:flex;align-items:center;justify-content:center;}.webcam-container img{width:100%;border-radius:8px;}.captured-container{border:2px solid #4a6cf7;border-radius:10px;padding:10px;background:#f8f9fa;}</style>', unsafe_allow_html=True)
...
if st.session_state.page == "Home":
    ...
    with c2:
        ...
        if st.session_state.saved_captures:
            ...
            for i, path in enumerate(st.session_state.saved_captures[-5:]):
                st.write(f"{i+1}. {os.path.basename(path)}")
            if st.button("🗑️ Clear History"):
                st.session_state.saved_captures = []
                st.rerun()
                    ...
                    with st.spinner("Processing..."):
                        ...
                        if frames:
                            ...
                            with col_info:
                                ...
                                for d in frame_data['detections'][:5]:
                                    st.write(f"- {d.get('class_name', 'object')} ({d.get('confidence', 0):.2f})")
                            c1,c2,c3 = st.columns(3)
