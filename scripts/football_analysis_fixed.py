"""
Fixed Football Analysis for World Cup 2026
Portugal vs Uzbekistan - with Proper Goal Detection
"""
...
class FootballAnalyzer:
    def __init__(self, team_a_name="Portugal", team_b_name="Uzbekistan"):
        ...
        # Goal detection parameters - FIXED
        self.goal_area_left = (0, 200, 80, 400)
        self.goal_area_right = (560, 200, 640, 400)
        self.goal_history = []
        self.goal_cooldown = 0  # Prevent multiple detections of same goal
        self.goal_cooldown_frames = 60  # Wait 60 frames between goals
        self.ball_in_goal_frames = 0  # Count consecutive frames ball is in goal
        self.goal_required_frames = 5  # Require ball in goal for 5+ frames
    ...
    def analyze_frame(self, frame):
        ...
        # 6. Detect goals - FIXED
        if ball_position:
            goal_detected = self._detect_goal_fixed(ball_position)
            if goal_detected:
                ...
                self.goal_cooldown = self.goal_cooldown_frames  # Set cooldown
    ...
    def _detect_goal_fixed(self, ball_position):
        """Detect if ball is in goal area - FIXED with cooldown and frame counting"""
        if ball_position is None or self.goal_cooldown > 0:
            return None
        
        x, y = ball_position
        h, w = 480, 640
        
        goal_left = (x < 80 and y > 200 and y < 400)
        goal_right = (x > 560 and y > 200 and y < 400)
        
        if goal_left or goal_right:
            self.ball_in_goal_frames += 1
            self.goal_history.append((ball_position, self.frame_count))
            
            # Need ball in goal for multiple frames to confirm
            ...
            # Ball left goal area - reset counter but keep some history
            if self.ball_in_goal_frames > 0 and self.ball_in_goal_frames < self.goal_required_frames:
                # Ball was in goal but not enough frames - reset
                self.ball_in_goal_frames = 0
            elif self.ball_in_goal_frames >= self.goal_required_frames:
                # We already counted this goal
                pass
            else:
                self.ball_in_goal_frames = 0
    ...
    def _annotate_frame(self, frame, team_a, team_b, field_lines, ball_position, stats):
        ...
        # Draw goal areas
        cv2.rectangle(result, (0, 200), (80, 400), (0, 0, 255), 2)
        cv2.rectangle(result, (560, 200), (640, 400), (0, 0, 255), 2)
        
        # Stats overlay
        y_pos = 30
        cv2.rectangle(result, (0, 0), (300, 180), (0, 0, 0), -1)
        cv2.rectangle(result, (0, 0), (300, 180), (255, 255, 255), 1)
...
def analyze_worldcup_video(video_path, team_a="Portugal", team_b="Uzbekistan"):
    """Analyze a World Cup video with team names"""
    ...
    progress = tqdm(total=min(total_frames, 1000), desc="Processing")  # Process first 1000 frames for speed
    
    frame_count = 0
    goals_detected = []
    
    while cap.isOpened() and frame_count < 1000:  # Limit to 1000 frames for testing
        ret, frame = cap.read()
        if not ret:
            break
    ...
    for goal in goals_detected[:10]:  # Show first 10 goals
